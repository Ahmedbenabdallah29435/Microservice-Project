...
export class ServicePlanService {
  ...
  private BASE_URL2 ='http://localhost:8084/api/plans/add'
  private BASE_URL_REVENU ='http://localhost:8084/api/plans/revenueoffer'
  private BASE_URL1 = 'http://localhost:8084/api/plans/getPlan';
  ...
  addPlan(Offer: Offer, id:number): Observable<any> {
    return this.http.post<Offer>(`${this.BASE_URL_AJOUT}/${id}` ,Offer)
  }


  getOffreById(id :number): Observable<Offer>{
    console.log('gg' , id)
    return this.http.get<Offer>('http://localhost:8084/api/plans/'+id)
  }

  updateplan(data:any){
    return this.http.put('http://localhost:8084/api/plans/updatePlan' ,data)
  }
